...
(function (root, factory) {
	...
	JSMongoQuery = {
		 ...
		 * @throws Exception
		 */
		executeQuery: function($query, $document, $options) {
			$options = $options || {};
		 ...
		 * @throws Exception
		 */
		_executeQuery: function($query, $document, $options, $logicalOperator) {
			if(!$logicalOperator) {
				$logicalOperator = '$and';
			}
		 ...
		 * @throws Exception
		 */
		_executeQueryOnElement: function($query, $element, $document, $options) {
			if($options.debug && $options.logger) {
		 ...
		 */
		_executeOperatorOnElement: function($operator, $operatorValue, $element, $document, $options) {
			if($options.debug && $options.logger) {
		 ...
		 */
		appendFieldSpecifier: function($query, $append) {
		 ...
		 */
		getDependentFields: function($query) {
			var $fields = [];
			_.each($query, function($v, $k) {
				if(is_array($v))
