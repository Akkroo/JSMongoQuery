...
(function (root, factory) {
	...
	var count = _.size,
		...
		is_int = function(n) { return typeof n === 'number' && n % 1 == 0; },
	...
	JSMongoQuery = {
			...
			switch($operator) {
				...
				case '$e':
					...
					if(is_string($operatorValue) && ($matches = $operatorValue.match(/^\/(.*?)\/([a-z]*)$/i)))
						return $v.match(new RegExp($matches[1], $matches[2]));
				...
				default:
					if(!is_callable($options['unknownOperatorCallback']))
		 ...
		 * @param $callable	A callback function on every field, signature function($field, $fieldQuery)
